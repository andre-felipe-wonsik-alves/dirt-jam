...
@export_range(0, 1) var fog_density : float = 0.02
@export_color_no_alpha var fog_color:Color = Color(0.6, 0.7, 0.8, 1.0)
...
func initialize_render(framebuffer_format : int):
	p_shader = compile_shader(read_file("res://Scripts/source_vertex.txt"), read_file("res://Scripts/source_fragment.txt"))
	p_wire_shader = compile_shader(read_file("res://Scripts/source_vertex.txt"), read_file("res://Scripts/source_wire_fragment.txt"))
...
func _render_callback(_effect_callback_type : int, render_data : RenderData):
	...
	# ANDREZAO: Adicionando a ModelMatrix, copiando o Acerola de novo hehehe
	var model_mat4 = Projection(model);
	for i in range(0,16):
		buffer.push_back(model_mat4[i / 4][i % 4])
